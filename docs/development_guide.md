...
Continuous integration is defined in `.github/workflows/ci.yml`: on every PR to `main` and every push to `main`, GitHub-hosted `ubuntu-latest` runners execute three independent jobs — `backend-node` (build + Jest), `backend-python` (pytest), and `frontend` (lint + format check + build + Vitest). No live services (Postgres/Redis/ChromaDB/Ollama) are started; the suite mocks them. `main` is now a protected branch (`openspec/changes/ci-branch-protection/`): all three checks must pass and be up to date with `main` before any pull request can merge, enforced even for the repository admin, and direct pushes to `main` are rejected. See `openspec/changes/archive/2026-08-28-ci-pipeline-foundation/` for the pipeline's original rationale and the remaining planned follow-up (e2e-in-CI).
