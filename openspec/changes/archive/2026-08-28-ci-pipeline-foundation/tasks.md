...
- [x] 4.4 Re-verified after all fixes (and after switching to `ubuntu-latest`, see §1): run `32760583997` — all 3 jobs passed, counts match local baseline exactly (71 Jest / 59 pytest / 49 Vitest)
- [x] 4.5 Pushed a scratch commit (`backend/api/__ci-scratch__.test.ts`, always-failing assertion) in run `32760725642`: `backend-node` correctly turned **failure**, `backend-python`/`frontend` correctly stayed **success**. Reverted; run `32760810414` back to all-green.
- [x] 4.6 Verification report created: `openspec/changes/ci-pipeline-foundation/specs/ci-pipeline/reports/2026-08-24-step-4-unit-test-and-pipeline-verification.md`
