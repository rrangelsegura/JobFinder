...
- Workflow runs on **GitHub-hosted `ubuntu-latest` runners** — free and unlimited for this public repo. (A self-hosted runner on the project owner's PC was tried first, found and fixed 3 real CI bugs, then was deregistered and removed after the owner raised a legitimate local-security concern; see `design.md` Decision 1 for the full rationale.)
...
1. **ci-pipeline-foundation** (this change): GitHub-hosted CI for the existing unit-test suite, informative only.
...
### New Capabilities
- `ci-pipeline`: Automated build/lint/test execution on GitHub-hosted runners for every PR and push to `main`, covering the Node backend, Python agentic core, and frontend, without starting any live service dependency.
...
- **New files**: `.github/workflows/ci.yml`, `.gitattributes` (normalizes checkouts to LF, needed once a real CI runner exposed a CRLF/autocrlf mismatch).
- **Infrastructure**: none — GitHub-hosted runners, nothing installed anywhere. (A self-hosted runner was provisioned and later fully removed during this change; no trace remains on the owner's machine or in GitHub's runner registry.)
